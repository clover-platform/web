...
export const Uploader = forwardRef<HTMLDivElement, UploaderProps>((props, ref) => {
    ...
    const renderFile = (file: File & any, index: number) => {
        return <div key={file.uid} className={cn(
            ...
            <div className={"flex-1 mx-1"}>
                ...
                { file.status === "error" && <div className={"flex justify-end items-center text-red-500"}><CrossCircledIcon className={"mr-1"}/> {file.error.message}</div> }
                { file.status === "done" && <div className={"flex justify-end items-center text-green-500"}><CheckCircledIcon/></div> }
            </div>
            <div
                className={cn(

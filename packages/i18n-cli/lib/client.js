...
export class I18nClient {
  ...
  async push() {
    ...
    for (const file of files) {
      ...
      data.toString().replace(/(?<![a-zA-Z])t\(["'`](.*?)["'`][),]/g, (_match, p1) => {
        // 去重 push
    ...
    const r = await axios
      .post(`${domain}/api/i18n/open/${module}/branch/${branch}/entry/push`, sourceObject, {
        headers: {
  ...
  async createBranchIfNotExists() {
    ...
    const r = await axios
      .post(
        `${domain}/api/i18n/open/${module}/branch/create/if/not/exist`,
        {
          name: branch,
          clone: main !== branch,
        },
  ...
  async pull() {
    ...
    for (const l of languages) {
      ...
      const r = await axios
        .get(`${domain}/api/i18n/open/${module}/branch/${branch}/entry/pull`, {
          params: {
            language: l.i18n,
          },
          headers: {

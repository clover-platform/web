...
export const Pagination: FC<PaginationProps> = (props) => {
    ...
    const itemClassName = classNames(
        textClassName,
        "hover:bg-white/20 hover:text-white/100 cursor-pointer"
    );
    const disabledClassName = "!bg-transparent !text-white/60 opacity-70";

    return <div className={classNames(
        "p-1 bg-black/50 rounded-md flex justify-center items-center",
        "hover:bg-black/70"
    )}>
        <div
            className={classNames(
                itemClassName,
                firstDisabled ? disabledClassName : null,
            )}
            onClick={() => {
        ...
        <div
            className={classNames(
                itemClassName,
                prevDisabled ? disabledClassName : null,
            )}
            onClick={() => {
        ...
        <div className={classNames(textClassName, "!text-white")}>
            { props.page }
        </div>
        <div
            className={classNames(
                itemClassName,
                nextDisabled ? disabledClassName : null,
            )}
            onClick={() => {
        ...
        <div
            className={classNames(
                itemClassName,
                lastDisabled ? disabledClassName : null,
            )}
            onClick={() => {

  ...
  return (
    <Dialog {...props} title={t('合并分支')} maskClosable={false}>
      <Loading loading={loading} className="space-y-4">
        <Alert icon={<IconBranch />}>
          <ul className="ml-4 list-disc text-muted-foreground leading-relaxed">
            <li>
              {t(
                '如果源字符串或标识符（key）被修改（匹配重复的标准），则会认为该字符串已被删除/添加（旧的删除，新的添加）。'
              )}
            </li>
        ...
        <div className="flex items-center space-x-2">
          <Checkbox checked={deleteAfterMerge} onCheckedChange={setDeleteAfterMerge} id="terms" />
          <label
            htmlFor="terms"
        ...
        <Space className="justify-end">
          <Button onClick={merge} loading={submitting} type="button">
            {t('合并')}
          </Button>
